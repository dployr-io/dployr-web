    ...
    <div className="relative rounded border bg-background">
      <div className="absolute top-2 right-2 z-10 flex items-center gap-2">
        <div className="flex items-center gap-1 rounded-md bg-background p-1 shadow-sm border">
          <Button type="button" variant={blueprintFormat === "yaml" ? "default" : "ghost"} size="sm" onClick={() => setBlueprintFormat("yaml")} className="h-5 px-2 text-xs">
            YAML
          </Button>
          <Button type="button" variant={blueprintFormat === "json" ? "default" : "ghost"} size="sm" onClick={() => setBlueprintFormat("json")} className="h-5 px-2 text-xs">
            JSON
          </Button>
        </div>
