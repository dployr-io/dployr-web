        ...
        {Object.entries(config || {}).map(([key, value]) => {
          ...
          return (
            <TableRow key={key}>
              ...
              <TableCell>
                {editingKey === key ? (
                  <Input value={editValue} onChange={e => setEditValue(e.target.value)} onKeyDown={e => handleKeyboardPress(e, key)} placeholder="Enter new value" className="h-8 w-full" autoFocus />
                ) : (
                  <span className="text-sm text-neutral-600">{isSet ? value : "Unset"}</span>
                )}
              </TableCell>
