  ...
  return {
    ...
    node: data.node
      ? {
          ...
          commit: data.node.commit,
          build_date: data.node.buildDate,
          go_version: data.node.goVersion,
          os: data.node.os,
          arch: data.node.arch,
        }
      : undefined,
...
function denormalizeToV1(data: NormalizedInstanceData): InstanceStreamUpdateV1 {
  return {
    ...
    instance_id: data.instance.tag,
    uptime: "",
    build_info: data.node
      ? {
          version: data.node.version,
          commit: data.node.commit,
          date: data.node.buildDate,
          go_version: data.node.goVersion,
        }
      : undefined,
    platform: data.node
