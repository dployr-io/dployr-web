...
import type { FsNode } from "../v1/entities/filesystem";
 ...
 * v1 uses 'ws' instead of 'websocket'
 */
 ...
 * v1 stores CPU in 'top', memory/disk in 'debug.system'
 */
 ...
 * v1 has deployments with nested config and flat services
 */
 ...
 * v1 uses proxy_status and proxy_apps separately
 */
function normalizeProxy(update: InstanceStreamUpdateV1): NormalizedProxy | null {
  const proxyStatus = update.proxy_status;
  const proxyApps = update.proxy_apps;
  ...
  return {
    ...
    version: null,
    routeCount: proxyStatus?.routes || proxyApps?.length || 0,
 ...
 * v1 uses inline permissions instead of nested permissions object
 */
...
function normalizeFilesystem(
  filesystem: InstanceStreamUpdateV1["filesystem"]
 ...
 * v1 stores diagnostics in 'debug'
 */
